...
export const CallToAction = ({ctaImage, ctaTitle, ctaDescription, ctaButton}: CallToActionProps) => {
    function getImageSrc(img: string | { url?: string } | undefined): string | null {
        if (!img) return null;
    ...
    if (!imageSrc && !ctaTitle && !ctaDescription && !(ctaButton && ctaButton.url && ctaButton.title)) {
        return null;
    }
    ...
    return(
        <div className={s.root}>
            {imageSrc && (
                <Image src={imageSrc} alt={ctaTitle || "CTA image"} width={400} height={200} className={s.image}/>
            )}
            <div>
