...
export default function EditPost() {
	...
	useEffect(() => {
		...
		fetch(`/api/posts/${id}`)
			...
			.then(data => {
				...
				setComments(
					(data.comments || []).map(
						(c: { author: string; date: string; text: string }) => ({
							...c,
							date: toInputDateFormat(c.date),
						})
					)
				);
	...
	return (
		<form className={s.wrapper} onSubmit={handleSubmit}>
			<PreservingLink href='/admin/posts'>Back to posts</PreservingLink>
			...
			<div>
				...
				<button
					...
					type='button'
					onClick={() =>
						setComments([...comments, { author: '', date: '', text: '' }])
					}
				>
					➕ Add Comment
				</button>
