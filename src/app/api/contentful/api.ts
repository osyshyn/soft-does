  ...
  return {
    ...
    posts: entries.items.map((item: any) => {
      ...
      return {
        ...
        image:
          item.fields.mainImage && assetMap[item.fields.mainImage.sys.id]
...
export const fetchFullBlogPostList = async (postId?: string) => {
  ...
  const entryMap = Object.fromEntries(
    (entries.includes?.Entry || []).map((e) => [e.sys.id, e])
  );
  const assetMap = Object.fromEntries(
    (entries.includes?.Asset || []).map((a) => [a.sys.id, a])
  );
        ...
        ? f.heroCarousel
            .map((ref: any) => {
              ...
              return {
                id: entry.sys.id,
                image:
                  entry.fields.image && assetMap[entry.fields.image.sys?.id]
                    ? {
                        url:
                          assetMap[entry.fields.image.sys.id]?.fields?.file
                            ?.url || null,
                        title:
                          assetMap[entry.fields.image.sys.id]?.fields?.title ||
                          null,
                        description:
                          assetMap[entry.fields.image.sys.id]?.fields
                            ?.description || null,
                      }
                    : null,
                url: entry.fields.url || null,
